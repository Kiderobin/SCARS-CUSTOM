...
function s.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
    if chk == 0 then
        return Duel.GetLocationCount(tp, LOCATION_MZONE) > 1
            and Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_DECK + LOCATION_HAND, 0, 2, nil, e, tp)
    end
...
function s.atklimit(e, tp, eg, ep, ev, re, r, rp)
    local fieldSpell = Duel.GetFieldCard(tp, LOCATION_SZONE, 0)
    if fieldSpell and fieldSpell:IsCode(43175858) then
        ...
        -- Apply the existing attack limit effect
