...
s.listed_names={1000000000,1000000011}
function s.cfilter(c,ft,tp)
	return c:IsCode(1000000011) and (ft>0 or (c:GetSequence()<5 and c:IsControler(tp))) and (c:IsFaceup() or c:IsControler(tp))
end
...
function s.filter(c,e,tp)
	return c:IsCode(1000000011) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_EXTRA,LOCATION_HAND,0,1,1,nil,e,tp)
